      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-5 relative z-10">
        <div className="flex items-start gap-4 flex-1 min-w-0 w-full">
          <div className="relative w-14 h-14 rounded-2xl overflow-hidden bg-white shadow-3d-sm shrink-0 border border-white/60 group/avatar">
            ...
            {canUpdatePhoto && (
              <button 
                onClick={(e) => { e.stopPropagation(); onCapturePhoto(member); }}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-1.5 mb-2.5">
              <Badge label={getRoleLabel(member.role)} variant={ADMIN_ROLES.includes(member.role) ? 'orange' : 'blue'} icon={ADMIN_ROLES.includes(member.role) ? ShieldCheck : undefined} className="scale-90 origin-left" />
              <Badge label={statusConfig.label} variant={statusConfig.variant} icon={statusConfig.icon} className="scale-90 origin-left" />
            ...
            {member.observations && (
              <div className="p-2 bg-blue-50/50 border border-blue-100/50 rounded-xl mb-3">
                <p className="text-[10px] font-black text-blue-700 uppercase tracking-tight leading-tight">{member.observations}</p>
              </div>
            )}
        ...
        <div className="flex flex-wrap sm:flex-col gap-2 w-full sm:w-auto border-t sm:border-t-0 border-slate-100/50 pt-4 sm:pt-0 relative z-20">
          {isAdmin && (
            <>
              <button onClick={(e) => { e.stopPropagation(); onEdit(member); }} className="btn-3d flex-1 sm:flex-none p-2.5 bg-white text-indigo-700 rounded-xl shadow-3d-sm border border-indigo-100/50 hover:bg-indigo-50 flex items-center justify-center gap-2 active:translate-y-0.5 transition-all min-w-[80px]">
              ...
              {onDelete && (member.uid !== currentUserId && member.id !== currentUserId) && (
                <button onClick={(e) => { e.stopPropagation(); onDelete(member); }} className="btn-3d flex-1 sm:flex-none p-2.5 bg-rose-500 text-white rounded-xl shadow-3d-sm border border-rose-400 hover:bg-rose-600 flex items-center justify-center gap-2 active:translate-y-0.5 transition-all min-w-[80px]">
                  <UserX size={16} /> <span className="sm:hidden text-[9px] font-black uppercase tracking-widest">Remove</span>
                </button>
              )}
            </>
          )}
          <button onClick={(e) => { e.stopPropagation(); onMessage(member.displayName || member.email, member.phoneNumber); }} className="btn-3d flex-1 sm:flex-none p-2.5 bg-emerald-500 text-white rounded-xl shadow-3d-sm border border-emerald-400 hover:bg-emerald-600 flex items-center justify-center gap-2 active:translate-y-0.5 transition-all min-w-[80px]">
