...
export default function MarketplaceModule() {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, 'marketplace'), { ...newItem, price: Number(newItem.price), sellerName: profile.displayName || 'Brother', sellerUid: profile.uid, whatsappLink: waLink, tenantId: profile.tenantId, createdAt: Date.now() });
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {filtered.map(item => <ProductCard key={item.id} item={item} category={CATEGORIES.find(c => c.id === item.category) || CATEGORIES[3]} isOwner={profile?.uid === item.sellerUid} onDelete={async (id) => { await deleteDoc(doc(db, 'marketplace', id)); showToast('Iklan dihapus'); }} />)}
