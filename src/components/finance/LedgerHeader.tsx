  ...
  return (
    <div className="flex bg-slate-100/50 p-1.5 rounded-[20px] mb-6 overflow-x-auto gap-2 border border-slate-200/50 backdrop-blur-md scrollbar-hide">
      <button
        ...
        className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.1em] transition-all shrink-0 border ${
          activeTab === 'ledger' 
      ...
      {isAdminRole && (
        <>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.1em] transition-all shrink-0 relative border ${
              activeTab === 'approvals' 
                ? 'bg-indigo-600 text-white border-indigo-500 shadow-3d-sm' 
                : 'bg-white/40 text-slate-500 border-white/80 hover:bg-white'
            }`}
